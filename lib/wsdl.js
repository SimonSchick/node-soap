...
MessageElement.prototype._getNestedLookupTypeString = function(element) {
  ...
  if (element.children.length > 0) {
    ...
    element.children.forEach(child => {
      const resolvedChildType = self._getNestedLookupTypeString(child).replace(/\^_/, '');
...
class WSDL {
  constructor(definition, uri, options) {
    ...
    process.nextTick(() => {
      ...
      self.processIncludes(err => {
        ...
        const services = self.services = self.definitions.services;
        if (services) {
          for (name in services) {
            services[name].postProcess(self.definitions);
          }
        }
        const { complexTypes } = self.definitions;
        if (complexTypes) {
        ...
        const { bindings } = self.definitions;
  ...
  _processNextInclude(includes, callback) {
    ...
    open_wsdl_recursive(includePath, options, (err, wsdl) => {
      ...
      if (wsdl.definitions instanceof DefinitionsElement) {
        _.mergeWith(self.definitions, wsdl.definitions, (a, b) => {
      ...
      self._processNextInclude(includes, err => {
        callback(err);
      });
    });
  }
  ...
  xmlToObject(xml, callback) {
    const self = this;
    const p = typeof callback === 'function' ? {} : sax.parser(true);
    let objectName = null;
    const root = {};
    const schema = {
      Envelope: {
    ...
    p.onopentag = function(node) {
      ...
      if (!objectName && top.name === 'Body' && name !== 'Fault') {
        ...
        if (!message) {
          try {
            ...
            const { portTypes } = self.definitions;
      ...
      if (xsiType) {
        ...
        const typeDef = self.findSchemaObject(typeURI, type.name);
        if (typeDef) {
    ...
    p.oncdata = function(text) {
      ...
      if (/<\?xml[\s\S]+\?>/.test(text)) {
        const top = stack[stack.length - 1];
        const value = self.xmlToObject(text);
  ...
  objectToXML(obj, name, nsPrefix, nsURI, isFirst, xmlnsAttr, schemaObject, nsContext) {
    ...
    if (nsURI && isFirst) {
      if(self.options.overrideRootElement && self.options.overrideRootElement.xmlnsAttributes) {
        self.options.overrideRootElement.xmlnsAttributes.forEach(attribute => {
    ...
    if (Array.isArray(obj)) {
      for (i = 0, n = obj.length; i < n; i++) {
        const item = obj[i];
        const arrayAttr = self.processAttributes(item, nsContext);
        const correctOuterNsPrefix = parentNsPrefix || ns; // using the parent namespace prefix if given

        const body = self.objectToXML(item, name, nsPrefix, nsURI, false, null, schemaObject, nsContext);
      ...
      for (name in obj) {
        ...
        const attr = self.processAttributes(child, nsContext);
        ...
        if (isFirst) {
          ...
          if (self.definitions.schemas) {
            if (schema) {
              const childSchemaObject = self.findChildSchemaObject(schemaObject, name);
              ...
              if (childSchemaObject &&
                ...
                */
                ...
                if (childSchemaObject.$type) {
                  ...
                  const typeURI = schema.xmlns[typePrefix] || self.definitions.xmlns[typePrefix];
                  childNsURI = typeURI;
                ...
                const completeChildParamTypeObject = self.findChildSchemaObject(
        ...
        const useEmptyTag = !value && self.options.useEmptyTag;
        if (!Array.isArray(child)) {
          // start tag
