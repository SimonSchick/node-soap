...
const { once, isObject } = require('lodash');
  ...
  _invoke(method, args, location, callback, options, extraHeaders) {
    ...
    function finish(obj, body, response) {
      ...
      if (!output) {
        ...
        callback(null, null, body, obj.Header, xml);
        return;
      }
    ...
    if (this.streamAllowed && typeof this.httpClient.requestStream === 'function') {
      ...
      req.on('response', response => {
        ...
        // When the output element cannot be looked up in the wsdl, play it safe and don't stream
